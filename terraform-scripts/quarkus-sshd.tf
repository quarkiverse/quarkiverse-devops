...
resource "github_repository_vulnerability_alerts" "quarkus_sshd" {
...
resource "github_team_membership" "quarkus_sshd" {
  for_each = { for tm in ["sunix", "zbendhiba", "gansheer"] : tm => tm }
  team_id  = github_team.quarkus_sshd.id
  username = each.value
  role     = "maintainer"
}
...
resource "github_app_installation_repository" "quarkus_sshd" {
