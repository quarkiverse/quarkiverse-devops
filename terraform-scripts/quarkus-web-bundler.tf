# Create repository
resource "github_repository" "quarkus_web_bundler" {
...
resource "github_repository_vulnerability_alerts" "quarkus_web_bundler" {
  repository = github_repository.quarkus_web_bundler.name
  enabled    = true
}

# Create team
resource "github_team" "quarkus_web_bundler" {
...
resource "github_team_repository" "quarkus_web_bundler" {
  team_id    = github_team.quarkus_web_bundler.id
  repository = github_repository.quarkus_web_bundler.name
  permission = "push"
}

# Add users to the team
resource "github_team_membership" "quarkus_web_bundler" {
