...
resource "github_repository_vulnerability_alerts" "quarkus_tekton" {
...
resource "github_team_membership" "quarkus_tekton" {
  for_each = { for tm in ["iocanel", "aureamunoz", "cmoulliard","matheusandre1"] : tm => tm }
  team_id  = github_team.quarkus_tekton.id
  username = each.value
  role     = "maintainer"
}
...
resource "github_app_installation_repository" "quarkus_tekton" {
